...
function ProductRecords({ products, onEdit, onDelete }: { products: Product[]; onEdit: (product: Product) => void; onDelete: (product: Product) => void }) {
  return <><section className="desktop-table table-panel" aria-label="Products"><div className="table-scroll"><table className="data-table"><thead><tr><th>Name</th><th>Price (LKR)</th><th>Stock</th><th><span className="sr-only">Actions</span></th></tr></thead><tbody>{products.map((product) => <tr key={product.id}><td>{product.name}</td><td className="money">{product.price}</td><td><span className={`stock-level ${product.stockCount === 0 ? "stock-level--empty" : product.stockCount <= 5 ? "stock-level--low" : ""}`}>{product.stockCount} units</span></td><td><ProductActions product={product} onEdit={onEdit} onDelete={onDelete} /></td></tr>)}</tbody></table></div></section><div className="record-cards">{products.map((product) => <article className="record-card" key={product.id}><div className="record-card__topline"><strong>{product.name}</strong><span className={`stock-level ${product.stockCount === 0 ? "stock-level--empty" : product.stockCount <= 5 ? "stock-level--low" : ""}`}>{product.stockCount} units</span></div><div className="record-card__amounts"><div><span>Price</span><strong>LKR {product.price}</strong></div></div><ProductActions product={product} onEdit={onEdit} onDelete={onDelete} /></article>)}</div></>;
}
